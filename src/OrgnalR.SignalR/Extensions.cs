...
public static class SignalRExtensions
{
    ...
    {
        private readonly GrainMessageObserver @delegate;


        public MessageObserverFactory(IGrainFactoryProvider grainFactory) => @delegate = new GrainMessageObserver(
            typeof(T).Name,
        ...
        ) => @delegate.SendAddressedMessageAsync(msg, cancellationToken);


        public Task SendAllMessageAsync(
            AnonymousMessage allMessage,
            CancellationToken cancellationToken = default
        ) => @delegate.SendAllMessageAsync(allMessage, cancellationToken);
    }


    public class MessageObservableFactory<T> : IMessageObservable<T>
    {
        private readonly GrainMessageObservable @delegate;
        ...
        public Task SubscribeToConnectionAsync(
            ...
            MessageHandle since = default,
            CancellationToken cancellationToken = default
        ) => @delegate.SubscribeToConnectionAsync(
            connectionId,
            messageCallback,
            onSubscriptionEnd,
            since,
        ...
        public Task UnsubscribeFromAllAsync(
            ...
            CancellationToken cancellationToken = default
        ) => @delegate.UnsubscribeFromAllAsync(subscriptionHandle, cancellationToken);


        public Task UnsubscribeFromConnectionAsync(
            string connectionId,
            CancellationToken cancellationToken = default
        ) => @delegate.UnsubscribeFromConnectionAsync(connectionId, cancellationToken);
    }
    ...
    public class OrgnalRHubLifetimeManagerFactory<T> : HubLifetimeManager<T> where T : Hub
    {
        ...
        {
            await (await @delegate).SendUserAsync(userId,
                methodName,
                args,
                cancellationToken);
        }
